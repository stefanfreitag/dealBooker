...
class Mailer {
  ...
  static void sendMail(DealType dealType, int amount, Unit unit, Product product, int price) {
    ...
    final String rawTextBody =
        Mailer.generateRawTextMessage(dealType, amount, unit, product, price);
    log.info("Raw email text: " + rawTextBody);

    final String htmlTextBody =
        Mailer.generateHtmlTextMessage(dealType, amount, unit, product, price);
    ...
    try {
      ...
      SendEmailRequest request =
          new SendEmailRequest()
              .withDestination(new Destination().withToAddresses(mailerConfig.getTo()))
              .withMessage(
                  new Message()
                      .withBody(
                          new Body()
                              .withText(new Content().withCharset("UTF-8").withData(rawTextBody))
                              .withHtml(new Content().withCharset("UTF-8").withData(htmlTextBody)))
                      .withSubject(
                          new Content().withCharset("UTF-8").withData(mailerConfig.getSubject())))
      ...
      log.error(
          "The email to "
              + mailerConfig.getTo()
              + " was not sent. Error message: "
              + exception.getMessage());
    }
  }

  static String generateRawTextMessage(
    ...
    return MessageFormat.format(
        "Hello trader,\r\n"
            ...
            + "\r\n"
            + "Best regards,\r\n"
            + "The Deal Booker service",
        params);
  }

  static String generateHtmlTextMessage(
      final DealType dealType,
      final int amount,
      final Unit unit,
      final Product product,
